...
def dump():
    ...
    if path.isfile(f):
        ...
        tp.update()
        fm = Frame(tp)
        fm.pack(side=TOP, fill=BOTH, expand=True)
        pgs = Progressbar(tp, orient=HORIZONTAL, mode='determinate', maximum=1)
        pgs.pack(side=TOP, fill=X)
        tvf = Frame(fm)
        tvf.pack(side=LEFT, fill=Y)
        tv = Treeview(tvf, selectmode=BROWSE, show='tree')
        tv.pack(side=LEFT, fill=Y, expand=True)
        sb = Scrollbar(tvf, orient=VERTICAL, command=tv.yview)
        sb.pack(side=RIGHT, fill=Y)
        tv.config(yscrollcommand=sb.set)
        stf = Frame(fm)
        stf.pack(side=RIGHT, fill=BOTH, expand=True)
        st = ScrolledText(stf, width=80, height=20, wrap='none')
